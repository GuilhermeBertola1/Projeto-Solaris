...
function renderizarResultados(estado) {
  ...
  containerResultados.innerHTML = estado.plantas.map((planta, indice) => `
    <article class="card-planta">
      ...
      <h3>${planta.nome}</h3>
      <p>Luz: ${planta.luz} | Dificuldade: ${planta.dificuldade}</p>
      <button type="button" data-adicionar data-indice="${indice}">Adicionar ao Jardim</button>
    </article>
  `).join('');
}
